...
CSV.foreach(Rails.root.join('config/youtuber_list/youtuber_channels_data.csv')) do |data|
  ...
  p data[1]
  unless Youtuber.exists?(url: data[1])
    youtuber = Youtuber.create(name: data[0], url: data[1])
    api_id = "https://www.googleapis.com/youtube/v3/search?part=id,snippet&channelId=#{youtuber.url}&key=AIzaSyBd1wcxiyGzgosb5Renoi4eH3hWph7hulY"
    ...
    Video.create(video_id: video_id, video_title: video_title, youtuber_id: youtuber.id, video_category_id: category.id)
  end
end
